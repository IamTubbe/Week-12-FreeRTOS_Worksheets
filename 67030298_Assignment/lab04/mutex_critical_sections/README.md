...
|-------|------------|--------|-----------|-------------|---------|
| 1 (With Mutex) | 161 | 0 | 5 | 100% | ข้อมูลปลอดภัย ส่วนใหญ่ไม่มีการชนกันของข้อมูล |
| 2 (No Mutex) | 178 | 0 | 8 | 100% | มี data corruption เพิ่มขึ้นจากการเข้าถึงพร้อมกัน |
| 3 (Changed Priority) | 210 | 0 | 12 | 100% | Task ลำดับสูงได้ resource ก่อน ทำให้เกิดความไม่สมดุลในการเข้าถึง |

### คำถามสำหรับการทดลอง
1. เมื่อไม่ใช้ Mutex จะเกิด data corruption หรือไม่?
    - อาจเกิด data corruption เพราะหลาย task เข้าถึงข้อมูลพร้อมกัน
2. Priority Inheritance ทำงานอย่างไร?
    - ให้ task ลำดับต่ำ รับ priority สูงขึ้นชั่วคราว เมื่อถือ resource ที่ task ลำดับสูงต้องใช้
3. Task priority มีผลต่อการเข้าถึง shared resource อย่างไร?
    - ได้สิทธิ์เข้าถึง shared resource ก่อน task ที่มี priority ต่ำ
