...
|-------|--------|------|---------|-------|-------|---------|
| 1 (ปกติ) | 57 | 34 | 77 | 19 | 187 | ระบบทำงานสมดุล ใช้งานทุกส่วนปกติ |
| 2 (ไม่มี Sensor) | 0 | 81 | 117 | 44 | 242 | Load ไปตกที่ User/Network มากขึ้น ทำให้ภาระระบบสูงขึ้น |
| 3 (Network เร็ว) | 73 | 43 | 99 | 25 | 240 | ประสิทธิภาพสูงขึ้น แต่โหลดโดยรวมยังสูง อาจมีคอขวดที่ User |

### คำถามสำหรับการทดลอง
1. Processor Task รู้ได้อย่างไรว่าข้อมูลมาจาก Queue ไหน?
    - เพราะ xQueueSelectFromSet() คืนค่า Queue handle ของตัวที่มีข้อมูลเข้ามา
2. เมื่อหลาย Queue มีข้อมูลพร้อมกัน เลือกประมวลผลอันไหนก่อน?
    - เลือก ตัวที่แจ้งเหตุการณ์ก่อน (ตามลำดับเวลา)
3. Queue Sets ช่วยประหยัด CPU อย่างไร?
    - ทำให้ Task Block รอ Queue เดียว แทนการเช็คหลาย Queue ตลอดเวลา
